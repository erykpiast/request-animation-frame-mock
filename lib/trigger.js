module.exports = (function() {
    ...
    function _trigger(time) {
        ...
        for (var i = 0, maxi = queueCopy.length; i < maxi; i++) {
            try {
                queueCopy[i](time);
            } catch(err) {
                _rethrow(err);
    ...
    function setMode(newMode, newParams) {
        if(newMode !== mode) {
            ...
            if(newMode === modes.MANUAL) {
                ...
                } else if('function' !== typeof newParams.timeFn) {
                    throw new TypeError('timeFn property of configuration object is not a function');
                ...
                requestedAnimationFrame = params.setInterval(function() {
                   _trigger(params.timeFn(previousTime, frameCount)); 
                }, params.time);
                ...
                requestedAnimationFrame = params.requestAnimationFrame(_trigger);
            } else {
